...
const fs = require("fs");
cache = require("../Data/varcache");
usercache = require("../Data/usercache");
...
module.exports.Message_Handle = async function (holder, msg, command, index, args) {
    Holder = holder;
    msg.args = args;
    serverVars = holder.serverVars;
    globalVars = holder.globalVars;
    var hasPermission = false;
    var action = command.actions[index];
    if (action.type === "Wait") {
        this.Wait_Handle(holder, msg, command, index, args, action);
    } else {
        await this.RunAction(holder.Bot, msg, action, holder);
        holder.callNextAction(command, msg, args, index + 1);
    }

    fs.writeFileSync(
        path.join(__dirname, "../Data/variables/servervars.json"),
    ...
    fs.writeFileSync(
        path.join(__dirname, "../Data/variables/globalvars.json"),
...
module.exports.Wait_Handle = async function (holder, msg, command, index, args, action) {
...
module.exports.GetMentionedUser_Handle = function (msg, client, action) {
    console.log("Mentioned user: ", msg.mentions.members.first());
};
...
module.exports.CheckIfStringContains_Handle = function (msg, client, action) {
    ...
    if (action.stringtocheck.includes(action.valuetocheck)) {
        ...
        Holder.callNextAction(passActions, msg, msg.args, 0);
    }
    else {
...
module.exports.CheckIfArrayContainsValue_Handle = function (msg, client, action) {
    ...
    if (action.varname.includes(action.value)) {
        passActions.actions = action.trueActions;
        this.Event_Handle(Holder, "", 0, "", varsEToPass, passActions);
    } else {
        passActions.actions = action.falseActions;
        this.Event_Handle(Holder, "", 0, "", varsEToPass, passActions);
    }
...
function ParseActionVariables(action, msg) {
    var holderVars = {};
    holderVars["CommandAuthor"] = msg.member;
    holderVars["CommandChannel"] = msg.channel;
    holderVars["CommandMessage"] = msg;
    holderVars["DefaultChannel"] = Functions.getDefaultChannel(msg.guild);
    holderVars["guild"] = msg.guild;

    var injectVariables = {};
    injectVariables.holderVars = holderVars;
        ...
        try {
            if (
                ...
                newVal = newVal
                    ...
                    .replace("${holderVars.CommandAuthor.user.dmChannel}", "@@MSG_AUTHOR@@")
                ...
                newVal = newVal.replace("${holderVars.CommandAuthor.user.avatarURL}", aurl);
                newaction[e] = newVal;
            }
        } catch (err) {
            Holder.logError({ level: "error", message: err.stack });
        }
    });
...
module.exports.AddRoleToUser_Handle = function (msg, client, action) {
    ...
    if (rolel.position >= botRole) {
        console.log("Bot must have a role higher than the one it is assigning");
    } else {
        if (!rolel) {
            console.log("Role: " + action.rolename + " does not exist");
        } else {
            if (action.user === "" || action.user == "undefined") {
                ...
                if (mem) {
                    ...
                    console.log("Could not find user with tag: " + action.user);
                }
            }
        }
    }
...
module.exports.SendRandomImage_Handle = function (msg, client, action) {
    ...
    if (!chan && channel != "") {
        console.log("No channel found with name: " + channel);
    } else if (channel == "" && msg != "") {
...
module.exports.KickUser_Handle = function (msg, client, action) {
    ...
    if (member) {
        ...
        console.log("Member to be kicked doesn't exist");
    }
};
...
module.exports.BanUser_Handle = function (msg, client, action) {
    ...
    if (member) {
        ...
        console.log("Member to be banned doesn't exist");
    }
};
            ...
            reaction.users.cache.forEach(user => {
                if (!user.bot) {
                    ...
                    if (ReactedMember && RoleToGive) {
                        ...
                        console.log("Could not find role or member to give");
                    }
                }
            });
        } else {
            console.log("Role not found for this reaction");
        }
    });
...
module.exports.GetRow_Handle = async function (msg, client, action) {
    ...
    const csvFile = fs.readFileSync(
        path.resolve(__dirname, "../Data/sheets/" + action.selectedsheet)
    );
...
module.exports.CheckUserPermissions_Handle = function (msg, client, action) {
    ...
    if (mem) {
        ...
        console.log("Member not found: " + action.user);
    }
};
