...
var usersFile = __dirname + "/../Data/user/user.json";
...
module.exports.Event_Handle = async function (holder, events, index, type, varsE, aEvent) {
  var client = holder.Bot;
  Holder = holder;
  var applicableEvent;
  guild = varsE.guild;
  varsEToPass = varsE;
  ...
  if (applicableEvent && breakFailure) {
    if (index < applicableEvent.actions.length) {
      ...
      this.Event_Handle(Holder, "", index + 1, "", varsE, applicableEvent);
    }
  } else {
...
async function SendMessage(client, action) {
  ...
  if (!chan && action.channelname != "") {
    console.log("No channel found with name: " + action.channelname + ". Action name: " + action.name);
  } else {
...
async function SendDirectMessage(client, action) {
  ...
  if (!member) {
    console.log("No user found for direct message");
  } else {
...
function ParseActionVariables(action) {
  ...
  holderVars["DefaultChannel"] = Functions.getDefaultChannel(guild);
  holderVars["guild"] = guild;
  var injectVariables = {};
...
function GetRow_Handle(client, action) {
  ...
  var objectToAdd = {};
  const csvFile = fs.readFileSync(path.resolve(__dirname, "../Data/sheets/" + action.selectedsheet));
...
function FindChannel(guild, channel) {
  return guild.channels.cache.find((ch) => ch.name === channel || ch.id === channel) || Holder.Bot.users.cache.get(channel);
}
